...
const checkPostalCode = function (e) {
  const constraints = {
    ...
    UK: {
      ...
      message:
        'UK postcode must be in a valid format (e.g., SW1A 0AA, G2 8PA).',
    },
    AU: {
      regex: /^\d{4}$/,
...
postalCodeField.addEventListener('blur', checkPostalCode)
...
confirmPasswordField.addEventListener('blur', () => {
  if (confirmPasswordField.value !== passwordField.value) {
    confirmPassErr.classList.add('confirm-password-err-active')
    confirmPassErr.textContent = "Passwords don't match"
  }
})
